...
function buildCar(isTaxi) {
  ...
  const track = (mat) => {
    ...
    car.userData.materials.push(mat);
    return mat;
  };
  ...
  const cabin = new THREE.Mesh(
    ...
    track(new THREE.MeshStandardMaterial({ color: 0x0a0c10, roughness: 0.2, metalness: 0.8 }))
  );
  ...
  if (isTaxi) {
    const sign = new THREE.Mesh(
      ...
      track(new THREE.MeshBasicMaterial({ color: 0xffe28a }))
    );
  ...
  const headMat = track(new THREE.MeshBasicMaterial({ color: 0xffffff }));
  headMat.color.multiplyScalar(3.5);
  const tailMat = track(new THREE.MeshBasicMaterial({ color: 0xff2222 }));
...
export function createTraffic(scene) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    scene.add(car);
    cars.push({ car, lane, speed: 6 + Math.random() * 5 });
  }
